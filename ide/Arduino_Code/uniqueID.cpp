...
#define SERIAL_NUMBER_WORD_3  *(volatile uint32_t*)(0x0080A048)
  ...
  *(uint32_t *)&FTFL_FCCOB3 = 0x41070000;
  ...
  // Renesas RA family (e.g. Arduino UNO R4) stores the 128-bit Unique ID in
  // Factory MCU Information Flash. FMIFRT holds the base address of the UIDR registers.
  // - FMIFRT address: 0x407F_B19C
  // - UIDR0..UIDR3 offsets: +0x14, +0x18, +0x1C, +0x20
  volatile uint32_t const * fmifrt = (volatile uint32_t const *)0x407FB19C;
  uint32_t const base = *fmifrt;

  volatile uint32_t const * uidr = (volatile uint32_t const *)(base + 0x14);
  uint32_t uid[4];
  uid[0] = uidr[0];
  uid[1] = uidr[1];
  uid[2] = uidr[2];
  ...
  for (uint8_t i = 0; i < 4; i++) {
    id[i * 4 + 0] = (uint8_t)(uid[i] >> 24);
    id[i * 4 + 1] = (uint8_t)(uid[i] >> 16);
    id[i * 4 + 2] = (uint8_t)(uid[i] >> 8);
    id[i * 4 + 3] = (uint8_t)(uid[i] >> 0);
  }
#endif
}
